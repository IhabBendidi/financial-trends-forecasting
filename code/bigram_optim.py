...
def create_bigram(period,db):
    ...
    for record in raw_data:
        ...
        for bc in unique_bigrams :
            ...
            output = {'bigram':bc[0],'tf':tf,'week':period[0],'year':period[1],'article':obj}
...
def create_tfidf(period,db):
    ...
    raw_data = db.tfidf.find({'year' : period[1],'week':period[0],'TF-IDF':None})
    for record in raw_data:
        #if record.get('TF-IDF')== None :
        tf = record.get('tf')
        b = record.get('bigram')
        idf,num_docs_of_bigram = compute_idf(b,period)
...
if __name__ == "__main__":
    ...
    for i in range(0,len(periods),8):
        p1 = multiprocessing.Process(target=compute_period,args=(periods[i+1],))
        p1.start()
        p2 = multiprocessing.Process(target=compute_period,args=(periods[i+2],))
        p2.start()
        p3 = multiprocessing.Process(target=compute_period,args=(periods[i+3],))
        p3.start()
        p4 = multiprocessing.Process(target=compute_period,args=(periods[i+4],))
        p4.start()
        p5 = multiprocessing.Process(target=compute_period,args=(periods[i+5],))
        p5.start()
        p6 = multiprocessing.Process(target=compute_period,args=(periods[i+6],))
        p6.start()
        p7 = multiprocessing.Process(target=compute_period,args=(periods[i+7],))
        p7.start()
        compute_period(periods[i])
